...
This is Part 9 of a 10-part series on query engine design. [Part 8](/blog/2026-04-29-query-engine-optimization-08-partitioning) covered partitioning. This article covers the most expensive operation in distributed query processing: joining two tables whose data lives on different nodes.
...
![Shuffle join redistributing both tables by join key hash so matching keys land on the same node](/images/blog/query-engine-optimization/09-distributed-joins-shuffle-join.png)
...
![Broadcast join copying the small dimension table to every compute node while the large fact table stays in place](/images/blog/query-engine-optimization/09-distributed-joins-broadcast-join.png)
...
![Hash join build and probe phases showing hash table construction from the smaller table and probing from the larger table](/images/blog/query-engine-optimization/09-distributed-joins-hash-join-mechanics.png)
