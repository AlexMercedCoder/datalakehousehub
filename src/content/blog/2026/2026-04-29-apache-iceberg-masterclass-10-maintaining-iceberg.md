...
This is Part 10 of a 15-part [Apache Iceberg Masterclass](/blog/2026-04-29-apache-iceberg-masterclass-01-table-formats). [Part 9](/blog/2026-04-29-apache-iceberg-masterclass-09-storage-degradation) covered how tables degrade. This article covers the four maintenance operations that keep Iceberg tables healthy and the three approaches to running them.

## The Four Maintenance Operations

![The four Iceberg maintenance operations: compaction, snapshot expiry, orphan cleanup, and manifest rewriting](/images/blog/apache-iceberg-masterclass/10-maintaining-iceberg-maintenance-operations.png)

### 1. Compaction (File Rewriting)

![Compaction merging 500 small files into 2 large files with identical data](/images/blog/apache-iceberg-masterclass/10-maintaining-iceberg-compaction-before-after.png)

Compaction reads small files, merges them into optimally-sized files (128-512 MB), and optionally re-sorts the data. It is the most impactful maintenance operation because it directly addresses the [small file problem](/blog/2026-04-29-apache-iceberg-masterclass-09-storage-degradation) and restores sort order effectiveness.
...
Compaction with sorting rewrites files so that column values are ordered, tightening the min/max statistics and making [file skipping](/blog/2026-04-29-apache-iceberg-masterclass-03-iceberg-metadata-performance) far more effective:
...
![Comparison of automated versus manual maintenance approaches](/images/blog/apache-iceberg-masterclass/10-maintaining-iceberg-auto-vs-manual-maintenance.png)
...
Build a monitoring layer that checks table health metrics ([Part 9](/blog/2026-04-29-apache-iceberg-masterclass-09-storage-degradation) diagnostics) and triggers maintenance only when thresholds are exceeded (e.g., average file size drops below 64 MB).
...
**Not monitoring after migration:** Tables migrated from Hive or other formats ([Part 15](/blog/2026-04-29-apache-iceberg-masterclass-15-migrating-to-iceberg)) often inherit poor file layouts. Run an immediate compaction pass after any in-place migration.

[Part 11](/blog/2026-04-29-apache-iceberg-masterclass-11-metadata-tables) covers how to query the metadata tables that power diagnostics.
