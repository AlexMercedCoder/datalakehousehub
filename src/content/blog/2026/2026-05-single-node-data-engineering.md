...
For developers working in Python, Rust, or JavaScript, DataFrames are the preferred API for data manipulation. While Pandas has been the standard in Python for a decade, it is single-threaded, has a high memory footprint (often requiring 5 to 10 times the dataset size in RAM), and does not support query optimization.
...
![MotherDuck Dual Execution model showing how queries are split by a Hybrid Planner between local laptop CPUs and MotherDuck Cloud Engines](/images/blog/single-node-data-engineering/motherduck-hybrid-execution.png)
